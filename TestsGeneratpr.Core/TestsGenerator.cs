    ...
    public class TestGenerator : ITestsGenerator
    {
        ...
        private static MemberDeclarationSyntax GenerateMethod(string methodName, string classVariableName, MethodDeclarationSyntax method, int generationTemplate)
        {
            ...
            else
            {
                ...
                Dictionary<int, string> equalAssertDict = new Dictionary<int, string>
                {
                    {1, "That"},
                    {2, "Equal"},
                    {3, "AreEqual"}
                };
                Dictionary<int, List<string>> identDict = new Dictionary<int, List<string>>
                {
                    {1,  new List<string> {"Is", "EqualTo"} },
                    {2, new List<string> {"", ""}},
                    {3, new List<string> {"", ""} }
                };

                Dictionary<int, Func<ArgumentSyntax>> assertDict = new Dictionary<int, Func<ArgumentSyntax>>
                {
                {1, () => {return                 SyntaxFactory.Argument(
                                        SyntaxFactory.InvocationExpression(
                                            SyntaxFactory.MemberAccessExpression(
                                                SyntaxKind.SimpleMemberAccessExpression,
                                                SyntaxFactory.IdentifierName(identDict[generationTemplate][0]),
                                                SyntaxFactory.IdentifierName(identDict[generationTemplate][1])))
                                        .WithArgumentList(
                                            SyntaxFactory.ArgumentList(
                                                SyntaxFactory.SingletonSeparatedList<ArgumentSyntax>(
                                                    SyntaxFactory.Argument(
                                                        SyntaxFactory.IdentifierName(expectedVarName)))))); } },
                {2, () => { return SyntaxFactory.Argument(
                                        SyntaxFactory.IdentifierName(expectedVarName)); } },
                {3, () => { return SyntaxFactory.Argument(
                                        SyntaxFactory.IdentifierName(expectedVarName)); } }
                };
                var resultAssertionStatement = SyntaxFactory.ExpressionStatement(
                    SyntaxFactory.InvocationExpression(
                        SyntaxFactory.MemberAccessExpression(
                            ...
                            SyntaxFactory.IdentifierName(equalAssertDict[generationTemplate])))
                    .WithArgumentList(
                        SyntaxFactory.ArgumentList(
                            SyntaxFactory.SeparatedList<ArgumentSyntax>(
                                new SyntaxNodeOrToken[]{
                                    ...
                                    assertDict[generationTemplate]() }))));
                                 

                statements.Add(resultAssertionStatement);
            }

            Dictionary<int, string> failAssertDict = new Dictionary<int, string>
            {
                {1, "Fail"},
                {2, "True"},
                {3, "Fail"}
            ...
            Dictionary<int, Func<ArgumentListSyntax>> failMsgDict = new Dictionary<int, Func<ArgumentListSyntax>>
                {
                {1, () => {return    SyntaxFactory.ArgumentList(
                            SyntaxFactory.SingletonSeparatedList(
                            SyntaxFactory.Argument(
                                SyntaxFactory.LiteralExpression(
                                    SyntaxKind.StringLiteralExpression,
                                    SyntaxFactory.Literal("autogenerated"))))); } },
                {2, () => { return SyntaxFactory.ArgumentList(
                        SyntaxFactory.SeparatedList<ArgumentSyntax>(
                           new SyntaxNodeOrToken[]{
                                SyntaxFactory.Argument(
                                    SyntaxFactory.LiteralExpression(
                                ...
                                SyntaxFactory.Argument(
                                    SyntaxFactory.LiteralExpression(
                                        SyntaxKind.StringLiteralExpression,
                                        SyntaxFactory.Literal("autogenerated")))})); } },
                {3, () => { return SyntaxFactory.ArgumentList(
                        SyntaxFactory.SingletonSeparatedList(
                            SyntaxFactory.Argument(
                                SyntaxFactory.LiteralExpression(
                                    SyntaxKind.StringLiteralExpression,
            ...
            var assertFailStatement = SyntaxFactory.ExpressionStatement(
                SyntaxFactory.InvocationExpression(
                    SyntaxFactory.MemberAccessExpression(
                        SyntaxKind.SimpleMemberAccessExpression,
                        SyntaxFactory.IdentifierName("Assert"),
                        SyntaxFactory.IdentifierName(failAssertDict[generationTemplate])))
                .WithArgumentList(
                    failMsgDict[generationTemplate]()));
